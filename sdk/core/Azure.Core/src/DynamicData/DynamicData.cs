    ...
    public sealed partial class DynamicData : IDisposable
    {
        ...
        private class JsonConverter : JsonConverter<DynamicData>
        {
            public override DynamicData Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
            {
...
#if NET6_0_OR_GREATER
                byte[] original = JsonSerializer.SerializeToUtf8Bytes(document, typeof(JsonDocument), options);
                return new DynamicData(new MutableJsonDocument(document, original).RootElement);
#else
                ...
                return new DynamicData(MutableJsonDocument.Parse(data).RootElement);
#endif
            }
