...
/// <summary>Provides an enumerable of server-sent events information.</summary>
...
public sealed class SseEnumerable<T> : IAsyncEnumerable<SseItem<T>>, IEnumerable<SseItem<T>>
{
    ...
    /// <summary>Spec: "Initialize event's type attribute to "message""</summary>
    ...
    internal SseEnumerable(Stream stream, SseItemParser<T> itemParser)
    {
        _stream = stream;
        _itemParser = itemParser;
    }

    /// <summary>Gets an enumerable of the server-sent events.</summary>
    public IEnumerator<SseItem<T>> GetEnumerator()
    {
        // Validate that the enumerable is only used for one enumeration.
        ThrowIfNotFirstEnumeration();

        return IterateEvents();

        IEnumerator<SseItem<T>> IterateEvents()
        {
    ...
    public IAsyncEnumerator<SseItem<T>> GetAsyncEnumerator(CancellationToken cancellationToken = default)
    {
        ...
        return IterateEvents(cancellationToken);

        async IAsyncEnumerator<SseItem<T>> IterateEvents(CancellationToken cancellationToken = default)
        {
            // Rent a line buffer. This will grow as needed. The line buffer is what's passed to the stream,
            // so we want it to be large enough to reduce the number of reads we need to do when data is
            // arriving quickly. (In debug, we use a smaller buffer to stress the growth and shifting logic.)
    ...
    private int GetNewLineLength()
    {
        ...
        return _lineBuffer.AsSpan(_newlineIndex).StartsWith(CRLF) ? 2 : 1;
    }
...
//#endif
    ...
    private void SkipStartingBomIfPresent()
    {
        if (_lineLength >= 3 && _lineBuffer.AsSpan().StartsWith(Utf8Bom))
        {
            _lineOffset += 3;
            _lineLength -= 3;
        }
